...
ALGORITHM = "HS256"
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="token")  # built-in that matches /token to send back requested token
...
def access_token_expire_minutes() -> int:
    """Function to make testing expiry time easier."""
    return 30
...
def create_access_token(email: str):
    ...
    jwt_data = {"sub": email, "exp": expire, "type": "access"}  # type = type of token (e.g. confirmation, access)
...
def get_subject_for_token_type(token: str, type: Literal["access", "confirmation"]) -> str:
    try:
        ...
        raise create_credentials_exception("Token has expired") from e  # ExpiredSignatureError caused HTTPException
    ...
    if token_type is None or token_type != type:
        raise create_credentials_exception(f"Token incorrect type, expected '{type}'")
...
# populate token automatically via dependency injection
