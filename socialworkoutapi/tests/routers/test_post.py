...
async def test_get_all_posts_sorting(async_client: AsyncClient, logged_in_token: str, sorting: str,
                                     expected_order: list[int]):
    await create_post("Test Post 1", async_client, logged_in_token)
    await create_post("Test Post 2", async_client, logged_in_token)
    response = await async_client.get("/posts/all", params={"sorting": sorting})
    assert response.status_code == 200
...
async def test_get_all_posts_sort_likes(async_client: AsyncClient, logged_in_token: str):
    ...
    response = await async_client.get("/posts/all", params={"sorting": "most_likes"})
    assert response.status_code == 200
...
async def test_get_all_posts_wrong_sorting(async_client: AsyncClient):
    response = await async_client.get("/posts/all", params={"sorting": "wrong_params"})
    assert response.status_code == 422
